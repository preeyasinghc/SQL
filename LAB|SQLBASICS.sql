...
SELECT * FROM LOAN;
## first get the highest payment
SELECT MAX(PAYMENTS) AS HighestPayment from loan;
##Now use that highest payment to get the Loan_id
SELECT LOAN_ID FROM LOAN WHERE PAYMENTS = '9910';
SELECT * FROM LOAN ORDER BY PAYMENTS DESC;
SELECT * FROM LOAN WHERE LOAN_ID = '6312';
...
SELECT * FROM LOAN;
SELECT * FROM LOAN WHERE DURATION = '60' ORDER BY AMOUNT ASC;
...
SELECT * FROM `ORDER` WHERE ACCOUNT_ID = 34;
...
SELECT * FROM `ORDER` WHERE ORDER_ID BETWEEN 29540 AND 29560;
...
SELECT * FROM `ORDER` WHERE ACCOUNT_TO = 30067122;
...
SELECT * FROM LOAN;
SELECT SUM(AMOUNT) FROM LOAN GROUP BY ACCOUNT_ID ORDER BY account_id LIMIT 10;
...
ELSE 'INCOMING'
END AS TRANSACTION_TYPE FROM TRANS;